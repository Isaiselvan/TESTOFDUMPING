...
static u_int64_t protocol_counter[NDPI_MAX_SUPPORTED_PROTOCOLS +
NDPI_MAX_NUM_CUSTOM_PROTOCOLS + 1];
static u_int64_t protocol_counter_bytes[NDPI_MAX_SUPPORTED_PROTOCOLS +
NDPI_MAX_NUM_CUSTOM_PROTOCOLS + 1];
static u_int32_t protocol_flows[NDPI_MAX_SUPPORTED_PROTOCOLS +
NDPI_MAX_NUM_CUSTOM_PROTOCOLS + 1] = { 0 };
...
static void help(u_int long_help) {
  printf("pcapReader -i <file|device> [-f <filter>][-s <duration>]\n"
     "           [-p <protos>][-l <loops>[0-d][-h][-t][-v <level>]\n\n"
         "Usage:\n"
         "  -i <file.pcap|device>     | Specify a pcap file to read packets
from or a device for live capture\n"
...
(live traffic capture only)\n"
         ...
         "  -h                        | This help\n"
         "  -v <1|2>                  | Verbose 'unknown protocol' packet
print. 1=verbose, 2=very verbose\n");
...
ndpi_VISIT which, int depth, void *user_data) {
  ...
  if((which == ndpi_preorder) || (which == ndpi_leaf)) /* Avoid
walking the same node multiple times */
    printFlow(flow);
}

static void node_print_known_proto_walker(const void *node, ndpi_VISIT
which, int depth, void *user_data) {
  struct ndpi_flow *flow = *(struct ndpi_flow**)node;
...
static void node_proto_guess_walker(const void *node, ndpi_VISIT
which, int depth, void *user_data) {
  struct ndpi_flow *flow = *(struct ndpi_flow**)node;
...
static int node_cmp(const void *a, const void *b) {
  ...
  if(fa->lower_ip < fb->lower_ip) return(-1); else { if(fa->lower_ip >
fb->lower_ip) return(1); }
  if(fa->lower_port < fb->lower_port) return(-1); else {
if(fa->lower_port > fb->lower_port) return(1); }
  if(fa->upper_ip < fb->upper_ip) return(-1); else { if(fa->upper_ip >
fb->upper_ip) return(1); }
  if(fa->upper_port < fb->upper_port) return(-1); else {
if(fa->upper_port > fb->upper_port) return(1); }
  if(fa->protocol < fb->protocol) return(-1); else { if(fa->protocol >
fb->protocol) return(1); }
                                       ...
                                       struct ndpi_id_struct **dst)
{
  ...
  idx = (lower_ip + upper_ip + iph->protocol + lower_port +
...
MAX_NDPI_FLOWS);
      ...
      struct ndpi_flow *newflow = (struct
ndpi_flow*)malloc(sizeof(struct ndpi_flow));
    ...
    if(flow->lower_ip == lower_ip && flow->upper_ip == upper_ip
       && flow->lower_port == lower_port && flow->upper_port ==
upper_port)
...
static unsigned int packet_processing(const u_int64_t time, const
                                      ...
                                      u_int16_t ipsize, u_int16_t rawsize)
{
  ...
  if ((frag_off & 0x3FFF) == 0) {
    ...
    protocol = (const
u_int32_t)ndpi_detection_process_packet(ndpi_struct, ndpi_flow,
(uint8_t *) iph, ipsize, time, src, dst);
  } else {
    static u_int8_t frag_warning_used = 0;

    if (frag_warning_used == 0) {
      printf("\n\nWARNING: fragmented ip packets are not supported and
will be skipped \n\n");
      frag_warning_used = 1;
    }
...
guessed_flow_protocols);
  ...
  for (i = 0; i <= ndpi_get_num_supported_protocols(ndpi_struct); i++) {
    if (protocol_counter[i] > 0) {
      printf("\t\x1b[31m%-20s\x1b[0m packets: \x1b[33m%-13llu\x1b[0m
bytes: \x1b[34m%-13llu\x1b[0m "
             "flows: \x1b[36m%-13u\x1b[0m\n",
             ndpi_get_proto_name(ndpi_struct, i), (long long unsigned
int)protocol_counter[i],
...
static void openPcapFileOrDevice(void)
{
  ...
  if((_pcap_handle = pcap_open_live(_pcap_file, snaplen, promisc, 500,
errbuf)) == NULL) {
...
static void pcap_packet_callback(u_char * args, const struct
...
DETECTION MIGHT NOT WORK CORRECTLY\n\n");
    ...
    v4_warning:
      if (ipv4_warning_used == 0) {
        printf("\n\nWARNING: only IPv4 packets are supported in this demo
(nDPI supports both IPv4 and IPv6), all other packets will be
discarded\n\n");
        ipv4_warning_used = 1;
      }
      return;
    ...
    if(decode_tunnels && (iph->protocol == IPPROTO_UDP) && ((frag_off
& 0x3FFF) == 0)) {
      ...
      if((sport == GTP_U_V1_PORT) || (dport == GTP_U_V1_PORT)) {
        ...
        if((((flags & 0xE0) >> 5) == 1 /* GTPv1 */) && (message_type == 0xFF
/* T-PDU */)) {
          ip_offset = ip_offset+ip_len+sizeof(struct ndpi_udphdr)+8 /* GTPv1
header len */;

          if(flags & 0x04) ip_offset += 1; /* next_ext_header is present */
          if(flags & 0x02) ip_offset += 4; /* sequence_number is present (it
also includes next_ext_header and pdu_number) */
...
void test_lib() {
  ...
  tot_usec = end.tv_sec*1000000 + end.tv_usec - (begin.tv_sec*1000000
+ begin.tv_usec);
  closePcapFile();
