    ...
    tcp_pair *ptp)
{
    ...
    time_t curT = ICIS_TIME;//;= curIntStarttime;
  //  printf("DEV: %d", ICIS_TIME);
    struct tm * curTimeInfo;
    char TimeBuf[300];
    curTimeInfo = localtime(&curT);
    strftime(TimeBuf, 100, "Splunk %F  %T", curTimeInfo);
...
//		       pba->syn_count, pba->fin_count),bufr));
    ...
    sprintf(tmpstr, "%s truncated_data_bytes=",TimeBuf); 
    /* tell how much data was NOT captured in the files */
    StatLineI("truncated_data_bytes=","bytes",
	      pab->trunc_bytes, pba->trunc_bytes);
    sprintf(tmpstr, "%s truncated_packets=", TimeBuf);
    StatLineI("truncated_data_bytes=","pkts",
	...
	StatLineF("RTT max","ms","%8.1f",
        ...
        sprintf(tmpstr, "%s RTT_avg=",TimeBuf);
	StatLineF((char*)tmpstr,"ms","%f",
...
StatLineI_LL(
    ...
    u_llong argright)
{
    char *format ="%d";// FS_ULL;
    StatLineFieldL(label,units,format,argleft,1);
...
StatLineF(
    ...
    double argright)
{
    StatLineFieldF(label,units,format,argleft,1);
    ...
    char *value)
{
	...
	*/
     ...
     printf("%s%s%s", labbuf, value, units);
}
