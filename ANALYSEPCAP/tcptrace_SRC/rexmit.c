           ...
           unsigned tcp_data_length)
{
	...
	if (ptcb->rtt_max < etime_rtt)
        ...
        /* Flow Based: Added for grouping RTT stats*/
        rtt_ms = etime_rtt/1000;
        if(etime_rtt < 101)
        rttIndex = p0to100;
        else if(etime_rtt < 251)
        rttIndex = p101to250;
        else if(etime_rtt < 501)
        rttIndex = p251to500;
        else if(etime_rtt <  751)
        rttIndex = p501to750;
        else if(etime_rtt < 1001)
        rttIndex = p751to1000;
        else
        rttIndex = gt1000;

        //rttIndex = rtt_ms/100;
        ptcb->rttCount[rttIndex] += 1;
        ptcb->rttBytes[rttIndex] += tcp_data_length;
	...
	u_long eff_win)
{
	...
	if (ack == (pseg->seq_lastbyte + 1)) {
	    ...
	    ret = rtt_ackin (ptcb, pseg, intervening_xmits,tcp_data_length);
	} else {
	    /* cumulatively ACKed */
	    ++ptcb->rtt_cumack;
	    ret = CUMUL;
